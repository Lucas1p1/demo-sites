...
(function($) {
	$.fn.javaCase = function(opt) {
		opt = jQuery.extend({
		my_speed : null,
		close_speed : 300,
		bg_alpha : 0.5,
		scrollImage : null,
		javaCase_next : 'piro_next_out',
		javaCase_prev :  'piro_prev_out',
		radius : 4,
		close_all : '.piro_close,.piro_overlay',
		slideShow : null,
		slideSpeed : null //slideshow timing in secs
		}, opt);		
		function start_javaCase() {
			var corners = 
				'<tr>'+					   
				'<td colspan="3" class="javaCase_up"></td>'+
				'</tr>'+	
				'<tr>'+	
				'<td class="t_l"></td>'+
				'<td class="t_c"></td>'+
				...
				'</tr>'+
				'<tr>'+
				'<td colspan="3" class="javaCase_down"></td>'+
				'</tr>';
			var window_height =  $(window).height();
			var bg_overlay = $(jQuery('<div class="piro_overlay"></div>').hide().css({'opacity':+opt.bg_alpha,'height':window_height+'px'}));
			var main_cont = $(jQuery('<table class="javaCase_content" cellpadding="0" cellspacing="0"></table>'));
			...
			var piro_prev = $(jQuery('<a href="#prev" class="'+opt.javaCase_prev+'"></a>'));
			var piro_next = $(jQuery('<a href="#next" class="'+opt.javaCase_next+'"></a>'));				
			$('body').append(bg_overlay).append(main_cont);
			main_cont.append(corners);
			$('.javaCase_up').append(piro_close);
			$('.javaCase_down').append(piro_nav);
			...
			main_cont.hide();
			var my_gall_classes = $("a[class^='javaCase_gall']");
			var map = new Object();
			...
			var gall_settings = new Array();
				for (var key in map) {
					...
					alert('For single image is recommended to use class javaCase');
			...
			$(piro_gallery).each(function(array) {
				...
				img.onerror = function (){
				  ...
				  $('.c_c div').append('<p class="err_mess">There seems to be an Error:&nbsp;<a href="#close" class="close_javaCase">Close javaCase</a></p>');
					$('.close_javaCase').bind('click',function() {
						$('.err_mess').remove();
				...
				img.onload = function() {
				  ...
				  $(opt.close_all).bind('click',function(c) {
					...
					if (c.keyCode === 27) {
						c.preventDefault();
						if($(img).is(':visible') || $('.c_c>div>p>a').is('.close_javaCase')){
					  piro_close.add(bg_overlay).add(main_cont).add(caption).add(piro_next).add(piro_prev).fadeOut(opt.close_speed);
					  main_cont.removeClass('loading');
						clearTimeout(timer);
					  	$(piro_gallery).children().removeAttr('class');
